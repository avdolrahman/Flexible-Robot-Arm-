  ...
  {
    {
      ...
      RTBIO_DriverIO(0, ANALOGOUTPUT, IOWRITE, 1, &test_P.AnalogOutput_Channels,
                     (real_T*)&test_RGND, &parm);
    }
  }

  /* S-Function Block: <Root>/ Potentiometer */
  {
    ANALOGIOPARM parm;
    ...
    RTBIO_DriverIO(0, ANALOGINPUT, IOREAD, 1, &test_P.Potentiometer_Channels,
                   &test_B.Potentiometer, &parm);
  }

  /* S-Function Block: <Root>/Strain Gauge  */
  {
    ANALOGIOPARM parm;
    parm.mode = (RANGEMODE) test_P.StrainGauge_RangeMode;
    parm.rangeidx = test_P.StrainGauge_VoltRange;
    RTBIO_DriverIO(0, ANALOGINPUT, IOREAD, 1, &test_P.StrainGauge_Channels,
                   &rtb_StrainGauge, &parm);
  }
}
...
RT_MODEL_test_T *test(void)
{
  ...
  /* Initialize timing info */
  {
    ...
    test_M->Timing.sampleTimes[1] = (0.01);
  ...
  rtmSetTFinal(test_M, 30.0);
  test_M->Timing.stepSize0 = 0.01;
  test_M->Timing.stepSize1 = 0.01;

  /* External mode info */
  test_M->Sizes.checksums[0] = (1098612085U);
  test_M->Sizes.checksums[1] = (1439333411U);
  test_M->Sizes.checksums[2] = (3905650420U);
  test_M->Sizes.checksums[3] = (719975396U);
  ...
  test_M->Sizes.numBlockPrms = (17);   /* Sum of parameter "widths" */
  return test_M;
}
