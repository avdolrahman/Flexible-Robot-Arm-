...
void test_output(void)
{
  ...
  /* Step: '<Root>/Step' */
  rtb_Sum1_idx_3 = test_M->Timing.t[1];
  if (rtb_Sum1_idx_3 < test_P.Step_Time) {
    test_B.Step = test_P.Step_Y0;
  } else {
    test_B.Step = test_P.Step_YFinal;
  }
  ...
  rtb_Sum1_idx_3 = test_P.M[3] * rtb_Sum2 + test_DW.UnitDelay_DSTATE[3];

  /* Gain: '<Root>/K' */
  test_B.K = ((test_P.K[0] * rtb_Sum1_idx_0 + test_P.K[1] * rtb_Sum1_idx_1) +
              test_P.K[2] * rtb_Sum1_idx_2) + test_P.K[3] * rtb_Sum1_idx_3;
  ...
  for (i = 0; i < 4; i++) {
    ...
    test_B.Gain3[i] += test_P.A[i + 12] * rtb_Sum1_idx_3;
  }
...
RT_MODEL_test_T *test(void)
{
  ...
  /* External mode info */
  test_M->Sizes.checksums[0] = (3282842385U);
  test_M->Sizes.checksums[1] = (4136187138U);
  test_M->Sizes.checksums[2] = (2892475552U);
  test_M->Sizes.checksums[3] = (4057044729U);
  ...
  test_M->Sizes.numBlocks = (24);      /* Number of blocks */
  test_M->Sizes.numBlockIO = (9);      /* Number of block outputs */
  test_M->Sizes.numBlockPrms = (61);   /* Sum of parameter "widths" */
  return test_M;
}
